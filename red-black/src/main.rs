...
fn main() {
    ...
    loop {
      ...
      cpy = Rc::new(RefCell::new(input));
      let temp = cpy.borrow();
      v  = temp.split(" ").collect();
      ...
      match v[0] {
        ...
        "delete" => {
          ...
          // tree.delete(v[1].as_str())
        },
        "count" => {println!("There are {:?} leaves", tree.count_leaves())},
        "height" => {},
        "IOprint" => {
          println!("Inorder traversal:");
          println!("{:?}", tree.inorder_traversal());
        },
        "empty" => {println!("The tree is empty: {}", tree.is_empty())},
        "print" => {println!("{}", RBTree::pretty_print(&tree.root));},
        _ => {}
      }
    }
